...
def create_known_face(name: str, pic_path: str):
    uid = uuid.uuid4().int
...
def read_known_faces_from_directory():
    faces = []
    for pic_path in os.listdir(KNOWN_FACES_DIRECTORY_PATH):
        name = Path(pic_path).name.split('.')[0]
        print(name)
        known_face = create_known_face(name, f"{KNOWN_FACES_DIRECTORY_PATH}/{pic_path}")
...
if __name__ == '__main__':
    ...
    while True:
        frame, rgb_small_frame = videoFaceDetector.capture_small_frame()
